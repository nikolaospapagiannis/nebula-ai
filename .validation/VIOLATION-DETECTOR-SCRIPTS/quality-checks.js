...
class QualityValidator {
  ...
  async validate() {
    ...
    if (changedFiles.length === 0) {
      logger.info('ℹ️  No changed files found to validate');
      return true;
    }

    logger.info(`📁 Validating ${changedFiles.length} files...\n`);
  ...
  getChangedFiles() {
    try {
      ...
      // If no staged files, check recent changes or all source files
      try {
        const recentOutput = execSync('git diff HEAD~1 --name-only --diff-filter=AM', { encoding: 'utf8' });
        const recentFiles = recentOutput.trim().split('\n').filter(file => 
        ...
        // If git fails, scan src directory
  ...
  checkFakeImplementations(file, content, lines) {
    const fakePatterns = [
      'fake', 'mock', 'placeholder', 'TODO: implement',
    ...
    lines.forEach((line, index) => {
      fakePatterns.forEach(pattern => {
        if (line.toLowerCase().includes(pattern.toLowerCase()) && 
            ...
            !line.includes('spec') &&
            !line.includes('//')) {
          this.addViolation(file, index + 1, 'BLOCKER', `Fake implementation detected: ${pattern}`);
        }
      });
    });
  }
  ...
  reportResults() {
    ...
    if (this.violations.length > 0) {
      logger.info(`\n❌ Critical Violations: ${this.violations.length}`);
      this.violations.forEach(v => {
        logger.info(`  ${v.file}:${v.line} - ${v.message}`);
      });
    }
    ...
    logger.info(`Total Warnings: ${this.warnings.length}`);
    logger.info(`Total Info: ${this.infos.length}`);
